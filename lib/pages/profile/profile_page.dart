...
class _ProfilePageState extends State<ProfilePage>
  ...
  Future<void> _loadAnalyticsData() async {
    try {
      ...
      print(
          '=== Loading analytics for user: ${_currentUser!.id}, role: ${_currentUser!.role} ===');

      if (_currentUser!.role == UserRole.teacher) {
        print('Fetching teacher analytics...');
        final analytics =
            await _analyticsService.getTeacherAnalytics(_currentUser!.id);
        print('Received teacher analytics: $analytics');
        if (mounted) {
          setState(() {
            _analyticsData = analytics;
          });
          print('Analytics data set in state');
        }
      } else {
        print('Fetching student analytics...');
        final analytics =
            await _analyticsService.getStudentAnalytics(_currentUser!.id);
        print('Received student analytics: $analytics');
        if (mounted) {
          setState(() {
            _analyticsData = analytics;
          });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      //   ),
      //   title: const Text('Profile jk', style: TextStyle(color: Colors.white)),
      //   centerTitle: true,
      // ),
      body: _isLoading
          ? _buildLoadingScreen()
  ...
  Widget _buildAchievementsSection() {
    if (_analyticsData == null) {
      print('Analytics data is null, returning empty widget');
      return const SizedBox.shrink();
    }
    if (_currentUser?.role == UserRole.teacher) {
      // Teacher analytics summary - using real-time data from database
      final totalCourses = _analyticsData!["total_courses"] ?? 0;
      final totalStudents = _analyticsData!["total_students"] ?? 0;
      final totalRevenue = _analyticsData!["total_revenue"] ?? 0.0;
      final recentActivities =
          _analyticsData!["recent_activities"] as List<dynamic>? ?? [];

      print('=== Building teacher achievements section ===');
      print('Total courses from analytics: $totalCourses');
      print('Total students from analytics: $totalStudents');
      print('Total revenue from analytics: $totalRevenue');
      print('Recent activities count: ${recentActivities.length}');
      print('Full analytics data: $_analyticsData');
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              ...
              children: [
                _buildTeacherStatCard('Courses', totalCourses, Icons.menu_book,
                    AppTheme.primaryBlue),
                _buildTeacherStatCard('Students', totalStudents, Icons.people,
              ...
              Material(
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ListTile(
                      leading: Icon(activity['icon'] ?? Icons.event,
                          color: activity['color'] ?? AppTheme.primaryBlue),
                      title: Text(
    ...
    // Student achievements branch continues as before
